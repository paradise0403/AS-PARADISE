...
class BombFactory:
    ...
    def __init__(self) -> None:
        ...
        self.regular_tex = bs.gettexture('bunnyColor')
        self.ice_tex = bs.gettexture('egg2')
        self.sticky_tex = bs.gettexture('eggTex2')
        self.impact_tex = bs.gettexture('scorch')
        self.impact_lit_tex = bs.gettexture('shadow')
        self.land_mine_tex = bs.gettexture('sparks')
        self.land_mine_lit_tex = bs.gettexture('smoke')
        self.tnt_tex = bs.gettexture('usersButton')
...
class Bomb(bs.Actor):
    ...
    def __init__(
        ...
        *,
        ...
        if self.bomb_type == 'ice_impact':
             self.blast_radius = blast_radius * 0.9 
        ...
        if self.bomb_type == 'land_mine':
            ...
            fuse_time = 15.0  # Impact bombs usually have long fuse as backup
            self.node = bs.newnode(
                'prop',
                delegate=self,
                attrs={
                    ...
                    'body': 'sphere',
                    'body_scale': self.scale,
                    'mesh': factory.impact_bomb_mesh,  # <--- Impact Model
                    'shadow_size': 0.3,
                    'color_texture': factory.ice_tex, # <--- Ice Texture on Impact Mesh
                    'reflection': 'powerup',
                    'reflection_scale': [1.5],
                    'materials': materials + (factory.impact_blast_material,), # <--- Impact behavior
                },
            )
            # Arm it quickly so it can explode on hit
            self.arm_timer = bs.Timer(
                0.15, bs.WeakCall(self.handlemessage, ArmMessage())
            )
    ...
    def explode(self) -> None:
        ...
        self._exploded = True
        if self.node:
            # Ice impact ke liye blast nature 'ice' rakhenge
            blast_nature = 'ice' if self.bomb_type == 'ice_impact' else self.bomb_type
            
            blast = Blast(
                position=self.node.position,
                velocity=self.node.velocity,
                blast_radius=self.blast_radius,
                blast_type=blast_nature, # <--- Modified
                source_player=bs.existing(self._source_player),
                hit_type=self.hit_type,
                hit_subtype=self.hit_subtype,
            ).autoretain()
        ...
        # Bomb node ko delete karna mat bhulna
