    ...
    def _show_team_score(self, pos_v: float, sessionteam: bs.SessionTeam,
        ...
        if len(self.teams) != 2:
            ZoomText(
                ...
                position=(150, pos_v),
                maxwidth=100,
                color=(1.0, 0.9, 0.5) if scored else (0.6, 0.8, 0.6),
                shiftposition=(-100, pos_v),
                shiftdelay=shiftdelay,
                flash=scored,
                trail=scored,
                h_align='left',
                jitter=1.0,
                trailcolor=(0.0, 1.0, 0.0, 0)
            ).autoretain()
        else:
            ZoomText(str(sessionteam.customdata['score']),
                     position=(-250, 190) if pos_v == 65 else (250, 190),
                     maxwidth=100,
                     color=(0.5, 0.7, 1.0) if scored else (0.8, 0.65, 1.0),
                     shiftposition=(-250, 190) if pos_v == 65 else (250, 190),
                     shiftdelay=shiftdelay,
                     flash=scored,
                     trail=scored,
                     ...
                     h_align='center',
                     jitter=1.0,
                     trailcolor=(1.0, 0.0, 0.0, 0)).autoretain()
...
def show_player_scores(self,
    ...
    def _scoretxt(text: str | bs.Lstr,
        ...
        Text(text,
             ...
             scale=scale,
             color = (1.0, 0.55, 0.0, 1.0) if highlight else 
             (0.75, 0.6, 0.9, 0.5),
